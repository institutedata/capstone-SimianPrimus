...
export const fetchAndSyncArtworks = async (objectIDs: number[]): Promise<void> => {
  for (const objectID of objectIDs) {
    try {
      ...
      if (existingArtwork) {
        console.log(`Artwork with objectID ${objectID} already exists in the database. Skipping.`);
        continue;
      }
      
      // Assuming that "Painting" is the exact classification string you are looking for
if (artworkData.primaryImage && artworkData.objectName === 'Painting') {
  const newArtwork = new Artwork({
    ...
    constituents: artworkData.constituents?.map((constituent: { constituentID: any; role: any; name: any; constituentULAN_URL: any; constituentWikidata_URL: any; gender: any; }) => ({
  ...
  console.log(`Artwork with objectID ${artworkData.objectID} saved to the database.`);
} else if (!artworkData.primaryImage) {
  console.log(`Artwork with objectID ${artworkData.objectID} does not have a primary image and will not be saved to the database.`);
} else if (artworkData.objectName !== 'Painting') {
  console.log(`Artwork with objectID ${artworkData.objectID} is not classified as a Painting and will not be saved to the database.`);
}
    } catch (error: any) {
      console.error(`Error fetching object details for ID ${objectID}:`, error.message);
      continue;
    }
  }
};
...
export const getRandomArtwork = async (
  ...
  try {
    const artwork = await Artwork.findOne({
      order: literal('RAND()'), // Use RAND() for MySQL
    });
    if (artwork) {
      res.json(artwork);
    } else {
